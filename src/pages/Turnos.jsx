...
export default function Turnos() {
  ...
  const turnsWithUsers = turns.map((t) => {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <BackgroundLayout>
        <Panel>
          <h2 className="text-2xl md:text-3xl font-serif-title text-center mb-8">Turnos</h2>

          {turnsWithUsers.length === 0 ? (
            <p className="text-center text-usb-text-gray py-8">No hay turnos registrados</p>
          ) : (
            <>
              {/* Header */}
              ...
              {turnsWithUsers.map((turn) => (
                <div key={turn.id} className="grid grid-cols-1 md:grid-cols-[80px_1fr_1fr_80px] gap-4 items-center py-3 border-b border-gray-100">
                  <div className="flex items-center gap-2 md:block">
                    <span className="md:hidden font-bold text-usb-dark text-sm">Turno:</span>
                    <span className="bg-usb-gray-input rounded-lg px-4 py-2 text-sm text-usb-dark inline-block">{turn.id}</span>
                  </div>
                  <div className="flex items-center gap-2 md:block">
                    <span className="md:hidden font-bold text-usb-dark text-sm">Sede:</span>
                    <span className="bg-usb-gray-input rounded-lg px-4 py-2 text-sm text-usb-dark inline-block w-full">{turn.sede}</span>
                  </div>
                  <div className="flex items-center gap-2 md:block">
                    <span className="md:hidden font-bold text-usb-dark text-sm">Usuario:</span>
                    <span className="bg-usb-gray-input rounded-lg px-4 py-2 text-sm text-usb-dark inline-block w-full">{turn.userName}</span>
                  </div>
